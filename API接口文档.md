...
- **接口**：`GET /auth/me`
- **鉴权**：登录
- **功能**：返回当前登录用户自己的资料和统计信息（文章数、粉丝数、关注数）。
...
- **功能**：文章广场列表，支持按作者、分类、关键字筛选，以及按时间/热度排序，分页返回。只返回公共且未隐藏的文章。
...
- **接口**：`GET /content/feed`（关注动态由内容广场聚合接口承载，见 7.1）
- **鉴权**：登录
- **功能**：返回当前登录用户关注的人发布的内容动态。该功能由"内容广场聚合"接口 `GET /content/feed` 承载，登录态下会优先返回关注对象的内容，分页返回。
...
- **接口**：`GET /files`
- **鉴权**：无需登录（登录态下会返回 `isLiked`/`isFavorited`）
- **功能**：文件广场列表，支持按作者、关联文章、分类、文件类型、关键字筛选，分页返回。未登录只能看到公共且未隐藏的文件；传 userId 时返回该作者的文件（本人可看到自己的私人文件）。
...
- **接口**：`GET /videos`
- **鉴权**：无需登录（登录态下会返回 `isLiked`/`isFavorited`）
- **功能**：视频广场列表，支持按作者、分类、关键字筛选，以及按时间/热度排序，分页返回。只返回公共且未隐藏的视频。

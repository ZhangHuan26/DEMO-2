          ...
          <table className="w-full text-left text-sm text-neutral-800 whitespace-nowrap">
            <thead className="bg-neutral-50 border-b border-neutral-200 text-neutral-600 text-xs font-bold font-mono">
              <tr>
                ...
                <th className="px-6 py-3.5">证据图片</th>
                <th className="px-6 py-3.5">状态</th>
                <th className="px-6 py-3.5">处理结果</th>
                <th className="px-6 py-3.5">举报时间</th>
                <th className="px-6 py-3.5">处理时间</th>
                <th className="px-6 py-3.5 text-right">操作</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-neutral-200">
              {loading ? (
                <tr><td colSpan={10} className="px-6 py-8 text-center text-neutral-500">正在加载举报审核队列...</td></tr>
              ) : reports.length === 0 ? (
                <tr><td colSpan={10} className="px-6 py-8 text-center text-neutral-500">暂无待处理的违规举报</td></tr>
              ) : (
                reports.map((r, idx) => (
                  <tr key={`report-row-${r.id ?? idx}-${idx}`} className="hover:bg-neutral-50/80 transition-colors">
                    ...
                    <td className="px-6 py-4 max-w-[200px] truncate text-neutral-600" title={r.reason}>{r.reason || '-'}</td>
                    <td className="px-6 py-4">
                      {(r as any).evidenceImages && (r as any).evidenceImages.length > 0 ? (
                        <div className="flex items-center gap-1">
                          {(r as any).evidenceImages.slice(0, 3).map((img: string, i: number) => (
                            <img
                              key={i}
                              src={img}
                              alt={`证据${i + 1}`}
                          ...
                          {(r as any).evidenceImages.length > 3 && (
                            <span className="text-[10px] text-neutral-400">+{(r as any).evidenceImages.length - 3}</span>
                          )}
                        </div>
                      ) : (
                    ...
                    <td className="px-6 py-4 text-neutral-500 text-xs max-w-[150px] truncate" title={(r as any).handleResult || ''}>
                      {(r as any).handleResult || '-'}
                    </td>
                    <td className="px-6 py-4 text-neutral-500 text-xs">{formatDate(r.createdAt)}</td>
                    <td className="px-6 py-4 text-neutral-500 text-xs">{formatDate((r as any).handledAt)}</td>
